...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.info("Basic Data Preprocessing Started")
    try:
        df.drop(columns = ['Unnamed: 2', 'Unnamed: 3', 'Unnamed: 4'], inplace = True)
        df.rename(columns = {'v1': 'target', 'v2': 'text'}, inplace = True)
        logging.info("Basic Data Preprocessing Completed")
        return df
...
def save_data(train_data: pd.DataFrame, test_data: pd.DataFrame, data_path: str) -> None:
    ...
    try:
        ...
        logging.info("Unexpected error occurred while saving the data")
        raise CustomException(e, sys)
...
def main():
    try:
        ...
        logging.info("Failed to complete data ingestion process")
        raise CustomException(e, sys)
